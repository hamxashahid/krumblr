...
RSpec.describe PostsController, type: :controller do
  ...
  let(:valid_attributes) { {title: 'test-post', body: 'test-post-body', blog_id: blog.id, user_id: user.id } }
  let(:invalid_attributes) { {title: nil, body: 'test-post-body' } }
  let(:valid_session) { {} }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) { {title: 'test-post', body: 'test-post-body-updated'} }

      it "updates the requested post" do
        post = Post.create! valid_attributes
        put :update, params: {id: post.to_param, post: new_attributes}, session: valid_session
        post.reload
        expect(post.body).to eq('test-post-body-updated')
      end
